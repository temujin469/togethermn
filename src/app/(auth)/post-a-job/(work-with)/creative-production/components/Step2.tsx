...
function Step2() {
  ...
  const [isMakeContract, setIsMakeContract] = useState(job?.creativeProduction?.isMakeContract);
  ...
  const form = useForm<z.infer<typeof jobShema>>({
    ...
    defaultValues: {
      useMediaTypes: job?.creativeProduction?.useMediaTypes || [],
      // howLongUse: job?.creativeProduction?.howLongUse,
    }
  })

  const handleSubmit = ({ useMediaTypes }: z.infer<typeof jobShema>) => {
    setJob({...job, creativeProduction: { ...job?.creativeProduction, useMediaTypes, isMakeContract } })
    nextStep()
  }

...
function Step1() {
  ...
  const handleSubmit = ({ title, description, files,  locations, additionalMaterial }: z.infer<typeof jobShema>) => {
    // console.log(values)

    setJob({...job, title, description, files, additionalMaterial, isAdditionalMaterial, locations })
    nextStep()
  }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <FileInput onChange={field.onChange} files={field.value as any} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}

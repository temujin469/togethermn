...
import useGetBookedJobs from '@/hooks/job/useGetBookedJobs';
...
function BookedJobs() {
  ...
  const { data, isError, isLoading } = useGetBookedJobs({ variables: { token, bookedUserId: user?.id, page } })

  const bookedJobs = data?.data;
  const pagination = data?.meta.pagination;
  console.log(bookedJobs)


  // used for mutation //
  const poulateQuery = QueryString.stringify({
    fields: ["bookedJobs"],
    populate: {
      bookedJobs: {
        fields: ["id"]
  ...
  const updateMutation = useMutation({
    mutationFn: async (currentJobId: number) => {
      ...
      if (user?.id) {
        ...
        const populatedUserRes = await myApi.get<Partial<User>>(`/api/users/${user?.id}?${poulateQuery}`, {
          headers: {
            Authorization: "Bearer " + token
          }
        });

        // remove the current user from booked job lists //
        const bookedJobIds = populatedUserRes.data.bookedJobs?.map(job => job.id);
        const updatedBookedJobIds = bookedJobIds?.filter(id => id !== currentJobId);


        const res = await myApi.put(`/api/users/${user?.id}`, { bookedJobs: updatedBookedJobIds }, {
          headers: {
            Authorization: "Bearer " + token
          }
        });
    ...
    onSuccess: () => {
      toast({
        description: "Ажлын түүх устгагдлаа",
      })
      queryClient.invalidateQueries(useGetBookedJobs.getKey())
    },
    onError: () => {
      toast({
  ...
  const handleDeleteBookedJobHistory = (id: number) => {
  ...
  if (!isLoading && !Boolean(bookedJobs?.length)) return <EmptyStatus/>

  return (
    <div>
      {
        bookedJobs?.map(({ attributes, id }) =>{
          ...
          return (
            <div className='flex bg-white p-4 rounded-md flex-col gap-5 sm:flex-row justify-between items-center pb-5 mb-5 shadow'>
              ...
              <div className='flex gap-5 w-full sm:w-auto'>
                {/* <Button className='flex-[1]' onClick={() => handleDeleteBookedJobHistory(id)}>
                Устгах
              </Button> */}
